...
class linkdb(object):
    ...
    def __init__(self, conn, close=1):
        self.conn = conn
        # 创建连接（连接数据库）
        # cursor = conn.cursor()   #创建游标
        self.cursor = conn.cursor(cursor=pymysql.cursors.DictCursor)  # 设置游标格式为字典格式，即取值时会以字典的形式呈现
        self._close = close
